...
## 6. CANLI ÖLÇÜM VERİSİ (Bakım Modülü — sensör veya API)
Esnek yapı: her ölçüm ayrı satır. Sistem hiçbir parametreyi zorunlu tutmaz;
firma neyi gönderirse onu okur. Az veri = çalışır, çok veri = daha akıllı teşhis.

Her satır bir ölçüm:
- makine_id (PRES-01)
- zaman (tarih + saat)
- parametre_adi (titresim / yag_sicakligi / motor_sicakligi / motor_akimi / basinc / devir)
- deger (sayı)
- birim (mm/s, °C, A, bar, rpm)

### ÇEKİRDEK PARAMETRELER (her makinede aranır)
- titresim (mm/s) — mekanik arıza: rulman, balanssızlık, hizalama
- yag_sicakligi (°C) — hidrolik/yağlama sağlığı
- motor_sicakligi (°C) — soğutma, aşırı yüklenme
- motor_akimi (A) — makine zorlanması, yük artışı (en ucuz, en güçlü teşhis)

### TİPE GÖRE OPSİYONEL
- basinc (bar) — hidrolik/pnömatik makine (pres, enjeksiyon)
- devir (rpm) — kesici takım makinesi (CNC, torna)

NOT: Excel yükleme de aynı yapıyı kullanır (sütunlar: makine_id, zaman, parametre_adi, deger, birim).
Sensör/API varsa canlı akar, yoksa Excel'den aynı formatta gelir — modül farkı hissetmez.